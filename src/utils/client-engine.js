    ...
    $e.fn.extend({
        ...
        request: function (amn, type, name, adosname, jsondata, options) {
            ...
            var settings = {
                ...
                _amgn: this._amgn ? this._amgn: options['_amgn'],
                _amn: amn,
                _name: name,
                _type: type,
                _hasdata: (data ? "1" : "0"),
                _checkid: $e.getEnv('_checkid') ? $e.getEnv('_checkid'): options['_checkid']
            };
        ...
        loadData: function (s) {
            if (s) {
                ...
                if (msg) {
                    if (msg['code']==111){
                        mui.alert('请求超时，请重新登录！', function () {
                            plus.runtime.quit();
                        });
                    }else if (msg['message'] !== undefined){
                        this.showMessage('系统提示', msg['message']+'');
                    }else{
...
//         console.log('=============parent========' + options.isParent);
    ...
    $e.openWindow = function (id, url, options) {
        ...
        setting.url += '?v='+new Date().getTime();
    ...
    //$e._baseURI = 'http://60.173.9.77:15580/bfc_erp/';
